...
export class ConeRenderer extends AbstractRenderer {
  protected buffer = new GeometryBuffer(110);
  ...
  onInit() {
    ...
    this.gl.vertexAttribPointer(normal, 3, this.gl.FLOAT, false,  0, 90 * Float32Array.BYTES_PER_ELEMENT);
    this.gl.enableVertexAttribArray(normal);

    const color = this.gl.getAttribLocation(this.shader.program, 'a_color');
    this.gl.vertexAttribPointer(color, 4, this.gl.FLOAT, false,  stride, (90 + 90) * Float32Array.BYTES_PER_ELEMENT);
    this.gl.enableVertexAttribArray(color);
    this.ext.vertexAttribDivisorANGLE(color, 1);

    const model = this.gl.getAttribLocation(this.shader.program, 'a_model');
    this.gl.vertexAttribPointer(model, 4, this.gl.FLOAT, false, stride, (90 + 90 + 4) * Float32Array.BYTES_PER_ELEMENT);
    this.gl.vertexAttribPointer(model + 1, 4, this.gl.FLOAT, false, stride, (90 + 90 + 4 + 4) * Float32Array.BYTES_PER_ELEMENT);
    this.gl.vertexAttribPointer(model + 2, 4, this.gl.FLOAT, false, stride, (90 + 90 + 4 + 8) * Float32Array.BYTES_PER_ELEMENT);
    this.gl.vertexAttribPointer(model + 3, 4, this.gl.FLOAT, false, stride, (90 + 90 + 4 + 12) * Float32Array.BYTES_PER_ELEMENT);
  ...
  onAdd(entity: Entity) {
    const arr = [
      ...
      ...entity.geometry.normals,
      ...entity.material.color.toArray(),
      ...entity.transform.matrix.toArray(),
    ];
    console.log(arr);
    entity.bufferIndex = this.buffer.add(arr);
  }
